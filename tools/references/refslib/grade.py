...
def looks_broken(title, markdown):
    ...
    if ((title or "").lower().startswith("error: the request could not be satisfied")
            or (len(markdown or "") < 2000 and "generated by cloudfront" in head
                and "request blocked" in head)):
        return "the source is a CloudFront request-blocked error page, not the cited document"
