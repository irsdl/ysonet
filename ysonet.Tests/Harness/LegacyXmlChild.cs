...
namespace ysonet.Tests
{
    ...
    /// ones, so the external-DTD gadgets (DataViewManagerXxe, DataSetXxe, XmlDocumentXxe
    /// variant 1, XmlDocumentSurrogateXxe) can be fired without touching machine-wide state.
    ///
    /// It reads the payload back through ysonet's own PayloadReader, so it covers every
    /// formatter the product can read and needs no per-gadget branch of its own.
    ///
    ...
    internal static class LegacyXmlChild
    {
        ...
        {
            return Run(exePath, formatter, payloadFile, null, timeoutMs);
        }

        /// <summary>
        /// The overload for DataContractJsonSerializer, whose document names no type at all, so
        /// the reader has to be told what to build the root as.
        /// <paramref name="dataContractJsonRootType"/> is an assembly qualified name the CHILD
        /// resolves, so no target assembly is loaded here.
        /// </summary>
        internal static string Run(string exePath, string formatter, string payloadFile,
            string dataContractJsonRootType, int timeoutMs)
        {
            string arguments = "\"" + formatter + "\" \"" + payloadFile + "\"";
            if (!string.IsNullOrEmpty(dataContractJsonRootType))
                arguments += " \"" + dataContractJsonRootType + "\"";
...
internal static class LegacyXmlRunner
{
    ...
    private static string _rootTypeName;

    private static int Main(string[] args)
    {
        if (args.Length < 2)
        {
            Console.Error.WriteLine(""usage: <formatter> <payload file> [DataContractJson root type]"");
            return 2;
        }

        _formatter = args[0];
        _raw = File.ReadAllBytes(args[1]);
        _rootTypeName = args.Length > 2 ? args[2] : null;
    ...
    private static void Deserialize()
    {
        try
        {
            // ONE shared reader - ysonet's own PayloadReader, the same map its -t path uses -
            // instead of a switch this file has to grow per gadget. It owns the BYTES vs TEXT
            // split per format, which is the trap that used to live here: handing a binary
            // stream to a text deserializer turns a fire cell into a silent no-op that looks
            // exactly like a payload that did not work. It also means a new gadget's formatter
            // is covered here the moment the product can read that format back.
            Type rootType = _rootTypeName == null ? null : Type.GetType(_rootTypeName, true);
            PayloadReader.Read(_raw, _formatter, rootType);
        }
        catch (Exception ex)
        {
            // A property setter, a serialization constructor, or an IObjectReference fixup can
            // all still throw AFTER the fetch they were asked to make, so a throw is not a
            // failure here. The listener in the parent decides.
