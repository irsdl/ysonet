...
namespace ysonet.Tests
{
    ...
    internal class Tests
    {
        ...
        private static void WizardEndToEnd()
        {
            ...
            keys.Type("ObjectDataProvider").Enter(); // picker filter + pick
            keys.Digit(2);                        // formatter menu -> Json.NET (index 1)
            keys.TypeLine("calc.exe");            // command
            keys.Enter();                         // rawcmd? -> No (default)
            keys.Enter();                         // variant option (skip)
            keys.Enter();                         // xamlurl option (skip)
            keys.Enter();                         // output format -> auto (default)
            keys.Enter();                         // output path (skip -> stdout)
        ...
        private static void WizardOutputToFile()
        {
            ...
            keys.Enter();                    // xamlurl (skip)
            keys.Enter();                    // output format auto
            keys.TypeLine(file);             // output path -> file
        ...
        {
            // Reach the command text prompt, press Esc: the flow cancels back to the
            // top menu (no payload), instead of being stuck at the prompt.
            var keys = new ScriptedKeyReader();
            keys.Enter();                            // top -> gadget
            keys.Type("ObjectDataProvider").Enter(); // picker
            keys.Digit(2);                           // formatter Json.NET
            keys.Escape();                           // Esc at the command prompt -> back to menu
        ...
        private static void WizardPluginPath()
        {
            ...
            keys.TypeLine("calc.exe");       // command option (value)
            keys.Enter();                    // test? No
