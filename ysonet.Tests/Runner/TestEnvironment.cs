...
namespace ysonet.Tests
{
    ...
    internal static class TestEnvironment
    {
        ...
        // A usable CLR 2 (.NET 2.0/3.0/3.5) on this machine: the in-box legacy C# compiler
        // builds a child AND that child reports Environment.Version 2.0.50727. Probed
        ...
        // A shared-folder victim that proves the original .NET Framework 4.0 private
        ...
        private static List<string> VerdictNotes()
        {
            ...
            if (Limited)
            {
                ...
                if (SkippedFor(Clr2Runtime))
                {
                    ...
                    notes.Add("optional Windows feature \".NET Framework 3.5 (includes .NET 2.0 and 3.0)\".");
                }
                if (SkippedFor(NetFx3xReferenceAssemblies))
                {
                ...
                if (SkippedFor(NetFx40Target))
                {
                    ...
                    notes.Add("the NET40 gadget cells are unverified. Configure "
                        + Net40Target.SharedDirectoryVar + " and start the test host in an isolated VM.");
                }
        ...
        // which runtime it is on. Nothing weaker will do: the .exe.config pin is not proof (a
        // config asking for v4.0 really does get CLR 4, and the shim rolls forward when CLR 2
        // is absent), and the registry says what is installed rather than what runs.
        //
        // Lazy like every other capability, so a NORMAL run never compiles anything here.
        private static CapabilityResult ProbeClr2Runtime()
        {
            string reason;
            string clr = LegacyClrChild.ProbeReportedClr(out reason);
            if (clr == null)
                return new CapabilityResult(Clr2Runtime, CapabilityState.Absent,
                    "no CLR-2 child could be built or run here: " + reason, 0);
            if (clr.StartsWith(LegacyClrLane.Clr2VersionPrefix, StringComparison.Ordinal))
                return new CapabilityResult(Clr2Runtime, CapabilityState.Present,
                    "a child compiled by the in-box legacy compiler reported CLR " + clr, 0);
            return new CapabilityResult(Clr2Runtime, CapabilityState.Absent,
        ...
        // separately from the CLR 2 files. Resolving the real reference list is the probe,
        // because that is exactly what the lane build does.
        private static CapabilityResult ProbeNetFx3xReferenceAssemblies()
        {
            var missingLanes = new List<string>();
            foreach (LegacyClrLane lane in LegacyClrLane.All)
            {
                if (lane.Tag == "20") continue;   // the 2.0 lane never needs them
                string missing;
                if (lane.ResolveReferences(out missing) == null)
                    missingLanes.Add(lane.Label + ": " + missing);
            }
            if (missingLanes.Count == 0)
                return new CapabilityResult(NetFx3xReferenceAssemblies, CapabilityState.Present,
