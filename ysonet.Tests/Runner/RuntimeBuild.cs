...
namespace ysonet.Tests
{
    ...
    // which gadget and plugin effects actually fired on it.
    //
    ...
    internal static class RuntimeBuild
    {
        ...
        private static readonly Dictionary<string, HashSet<string>> _pluginFired =
            new Dictionary<string, HashSet<string>>(StringComparer.OrdinalIgnoreCase);
        ...
        public static List<string> FiredPlugins()
        {
            lock (_pluginFired)
            {
                var list = new List<string>(_pluginFired.Keys);
                list.Sort(StringComparer.OrdinalIgnoreCase);
                return list;
            }
        }
        ...
        {
            return VersionsIn(_fired, gadgetName);
        }

        public static List<string> PluginFiredVersions(string pluginName)
        {
            return VersionsIn(_pluginFired, pluginName);
        }
        ...
        public static bool AnythingFired()
        {
            lock (_fired)
                lock (_pluginFired)
                    return _fired.Count > 0 || _pluginFired.Count > 0;
        }

        private static void Record(Dictionary<string, HashSet<string>> record, string name,
            string versionToken)
        {
            ...
            {
                HashSet<string> versions;
                if (!record.TryGetValue(name, out versions))
                {
                    versions = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
                    record[name] = versions;
                }
                if (!string.IsNullOrEmpty(versionToken)) versions.Add(versionToken);
            }
        }

        private static List<string> VersionsIn(Dictionary<string, HashSet<string>> record,
            string name)
        {
            lock (record)
            {
                HashSet<string> versions;
                if (name == null || !record.TryGetValue(name, out versions))
