...
namespace ysonet.Tests
{
    ...
    internal sealed class TestRunOptions
    {
        ...
        public bool Dos;
        public bool Oob;
        // LEGACY (opt-in, separate): deserialize payloads on CLR 2 (.NET 2.0/3.0/3.5) in a
        // child this suite compiles with the in-box legacy compiler. It stands alone like
        // OOB rather than requiring --full, so it can be run in seconds.
        public bool Legacy;
        // NET40 (opt-in, separate): send generated bytes through a shared folder to an
        // isolated victim that proves it is the original .NET Framework 4.0 runtime.
        public bool Net40;
        ...
        public const string LegacyVar = "YSONET_LEGACY_TESTS";
        public const string Net40Var = "YSONET_NET40_TESTS";
        ...
        public static TestRunOptions Parse(string[] args, Func<string, string> env, bool debuggerAttached)
        {
            ...
            o.Legacy = HasFlag(args, "--legacy") || env(LegacyVar) != null;
            o.Net40 = HasFlag(args, "--net40") || env(Net40Var) != null;
        ...
        public string DescribeTiers()
        {
            ...
            if (Full) tiers += "+FULL";
            if (Oob) tiers += "+OOB";
            if (Legacy) tiers += "+LEGACY";
            if (Net40) tiers += "+NET40";
            if (Dos) tiers += "+DOS";
