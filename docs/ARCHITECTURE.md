...
| **Minifiers/XmlMinifier.cs** (was `XmlHelper.cs`) | Minify/normalize XML payloads (Soap, Net/DataContract, XmlSerializer): dedupe namespaces, strip encodingStyle, XSLT whitespace strip, ref-id minification. Stays linear on big inline-assembly payloads (tens of thousands of `<s:Byte>` elements): the encodingStyle scan is guarded and NCName-bounded, and the XSLT "drop unused namespaces" pass skips the reserved `xml` namespace (which is in scope on every element and never emitted), avoiding an O(n^2) `//*` scan per element. | `Minify` (6 overloads, string & Stream), `XmlXSLTMinifier` |
