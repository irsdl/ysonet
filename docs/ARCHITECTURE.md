...
| **Minifiers/XmlMinifier.cs** (was `XmlHelper.cs`) | Minify/normalize XML payloads (Soap, Net/DataContract, XmlSerializer): dedupe namespaces, strip encodingStyle, XSLT whitespace strip, ref-id minification. The encodingStyle scan is guarded (skipped when no `encodingStyle` is present) and its prefix class is NCName-bounded, so it stays linear on inline-assembly payloads instead of O(n^2). | `Minify` (6 overloads, string & Stream), `XmlXSLTMinifier` |
