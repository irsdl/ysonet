...
resource: "https://codewhitesec.blogspot.com/2022/01/dotnet-remoting-revisited.html"
tags: [article, ysonet-reference, codewhitesec-blogspot-com]
generated:
  by: ysonet-refs/1
...
sources:
  - id: original
    resource: "https://codewhitesec.blogspot.com/2022/01/dotnet-remoting-revisited.html"
    title: .NET Remoting Revisited
    last_modified: 2022-01
also_at: []
authors: []
canonical_url: ""
cited_by:
  - "docs/dotnet-deserialization-research.md:22"
  - "docs/references.md:24"
commit: ""
content_sha256: c5f421f479c6e468648bba1daebf3831ffff0525f7fe15f21fd8e4fa1f28babe
depth: full
depth_reason: default
kind: article
language: ""
licence: unknown
original_url: "https://codewhitesec.blogspot.com/2022/01/dotnet-remoting-revisited.html"
published: 2022-01
publisher: codewhitesec.blogspot.com
publisher_english: ""
raw_sha256: eb67d3cbc056cac874e286e9d58e36b173770d0376866cdaa8582294d8e3ad7e
retrieved_from: "https://codewhitesec.blogspot.com/2022/01/dotnet-remoting-revisited.html"
retrieved_kind: preserved-copy
retrieved_utc: "2026-08-04T17:37:52+00:00"
slug: 2022-codewhitesec-blogspot-com-net-remoting-revisited
snapshot: ""
title_english: ""
---
...
**.NET Remoting Revisited** - Author not stated, codewhitesec.blogspot.com.

- Published: 2022-01
- Original: <https://codewhitesec.blogspot.com/2022/01/dotnet-remoting-revisited.html>
- Preserved from: https://codewhitesec.blogspot.com/2022/01/dotnet-remoting-revisited.html (preserved-copy) on 2026-08-04
- Licence: unknown
...
And although .NET Remoting has already been [deprecated with the release of .NET Framework 3.0 in 2009](https://docs.microsoft.com/en-us/previous-versions/dotnet/netframework-3.0/72x4h507(v=vs.85)) and is [no longer available on .NET Core and .NET 5+](https://docs.microsoft.com/en-us/dotnet/core/porting/net-framework-tech-unavailable), it is still around, even in contemporary enterprise level software products.
...
The .NET Remoting is fairly configurable. The following security aspects are built-in and can be configured using special [channel and formatter properties](https://docs.microsoft.com/en-us/previous-versions/dotnet/netframework-4.0/kw7c6kwc(v=vs.100)):
...
There are basically two attack modes:
  ...
  `raw` Exploit `BinaryFormatter`/`SoapFormatter` deserialization (see also [YSoSerial.Net](https://github.com/pwntester/ysoserial.net)) all others commands (see `-h`) Write a *FakeAsm* assembly to the server's file system, load a type from it to register it at the server to be accessible via the existing .NET Remoting channel. It is then accessible via .NET Remoting and can perform various commands.
...
Now the problem with the `--uselease` option is that the remote class needs to return an actual `ILease` object and not `null`. This may happen if the virtual `MarshalByRefObject.InitializeLifetimeService()` method is overriden. But the main principle of sending an `ObjRef` referencing an object on the attacker's server can be generalized with any method accepting a parameter. That is why we have added the `--useobjref` to *ExploitRemotingService* (see also [*Community Contributions*]() further below):
  ...
  `--useobjref` Call the `MarshalByRefObject.GetObjRef(Type)` method with an `ObjRef` as parameter value. Similarly to `--uselease`, the server calls `IConvertible.ToType(Type, IformatProvider)` on the proxy, which sends a remoting call to the attacker's server.
...
[![](https://blogger.googleusercontent.com/img/a/AVvXsEhhLduGd9RK2M_gIZ4De9-fV_L_295IJ81B8bZWj3gvKeW1jNbTsnW3iZdo_TMeSInlBIOjDsXCysebaCRtdfljbXCxlQjeYmfgjHqVnESf2PxupiMma0fJsPqV3oYXJkfRG2tPE1efNR0Ki8Iq-f67OnPkG-9GFX1KeFxo8LZ2j7RnkxaBUyEfuOea)](https://blogger.googleusercontent.com/img/a/AVvXsEhhLduGd9RK2M_gIZ4De9-fV_L_295IJ81B8bZWj3gvKeW1jNbTsnW3iZdo_TMeSInlBIOjDsXCysebaCRtdfljbXCxlQjeYmfgjHqVnESf2PxupiMma0fJsPqV3oYXJkfRG2tPE1efNR0Ki8Iq-f67OnPkG-9GFX1KeFxo8LZ2j7RnkxaBUyEfuOea)

## Conclusion

.NET Remoting has already been deprecated long time ago for obvious reasons. If you are a developer, don't use it and [migrate from .NET Remoting to WCF](https://docs.microsoft.com/en-us/dotnet/framework/wcf/migrating-from-net-remoting-to-wcf).
